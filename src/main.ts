...
function renderOpenDay(data: any) {
  ...
  // Add event listener for search icon button (click and hover)
  const searchIconBtn = document.getElementById("searchIconBtn");
  const searchBarDropdown = document.getElementById("searchBarDropdown");
  if (searchIconBtn && searchBarDropdown) {
    let searchBarTimeout: number | undefined;
    // Show on click
    searchIconBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      searchBarDropdown.classList.toggle("hidden");
      if (!searchBarDropdown.classList.contains("hidden")) {
    ...
    searchIconBtn.addEventListener("mouseenter", () => {
      ...
      searchBarDropdown.classList.remove("hidden");
      (document.getElementById("searchInput") as HTMLInputElement)?.focus();
    });
    searchIconBtn.addEventListener("mouseleave", () => {
      searchBarTimeout = window.setTimeout(() => {
        searchBarDropdown.classList.add("hidden");
      }, 300);
    });
    searchBarDropdown.addEventListener("mouseenter", () => {
      clearTimeout(searchBarTimeout);
    });
    ...
    document.addEventListener("click", (e) => {
      if (!searchBarDropdown.classList.contains("hidden")) {
        if (!searchBarDropdown.contains(e.target as Node) && e.target !== searchIconBtn) {
          searchBarDropdown.classList.add("hidden");
        }
      }
    });
